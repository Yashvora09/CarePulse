...
const Login = () => {
  ...
  // Go back to wherever the user tried to visit, or /profile
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-medical-blue/10 mb-4">
            <Activity className="w-8 h-8 text-medical-blue" />
          </div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('login_welcome')}</h1>
          <p className="text-slate-500 mt-2">{t('login_sub')}</p>
        </div>
        ...
        <div className="card p-8 shadow-xl">
          ...
          <form className="space-y-5" onSubmit={handleLogin}>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('login_email')}</label>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">{t('login_password')}</label>
                <a href="#" className="text-sm text-medical-teal hover:underline font-medium">{t('login_forgot')}</a>
              </div>
              <div className="relative">
            ...
            <button type="submit" className="w-full btn-primary py-3 flex items-center justify-center gap-2 group mt-4 text-base font-bold">
              {t('login_btn')} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </button>
          </form>
          ...
          <div className="mt-6 text-center text-sm text-slate-600 dark:text-slate-400 border-t border-slate-100 dark:border-slate-700 pt-5">
            {t('login_no_acc')}{' '}
            <Link to="/signup" className="text-medical-blue dark:text-medical-teal font-bold hover:underline">
              {t('login_signup_link')}
            </Link>
          </div>
        </div>
